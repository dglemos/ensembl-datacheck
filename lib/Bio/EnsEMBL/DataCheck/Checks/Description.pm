...
package Bio::EnsEMBL::DataCheck::Checks::Description;
...
use constant {
  NAME        => 'Description',
  DESCRIPTION => 'Check that imported names or descriptions contain only supported characters',
  DB_TYPES    => ['variation'],
  TABLES      => ['phenotype']
};
...
sub tests {
  ...
  my $diag = 'Variation Phenotype has empty description'; 
  my $sql = qq/
      SELECT *
      FROM phenotype
